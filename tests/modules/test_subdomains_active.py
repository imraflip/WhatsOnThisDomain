"""Runs shuffledns bruteforce against hackerone.com with a tiny wordlist
and asserts at least one resolved subdomain comes back. Needs shuffledns,
massdns, and the resolvers list on PATH."""

from __future__ import annotations

from pathlib import Path

import pytest
from sqlalchemy.ext.asyncio import async_sessionmaker, create_async_engine

from wotd.models import Base, Subdomain
from wotd.modules.subdomains_active import SubdomainsActiveModule
from wotd.scope import RuleType, Scope, ScopeRule
from wotd.store import create_target

TINY_WORDLIST = Path(__file__).parent.parent / "data" / "wordlists" / "dns_tiny.txt"
RESOLVERS = Path("/opt/wotd/resolvers.txt")


@pytest.mark.flaky(retries=3, delay=10)
async def test_active_enum() -> None:
    engine = create_async_engine("sqlite+aiosqlite:///:memory:")
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
    session_factory = async_sessionmaker(engine, expire_on_commit=False)

    async with session_factory() as session:
        target = await create_target(session, name="hackerone.com", root_domains=["hackerone.com"])
        scope = Scope(
            includes=[
                ScopeRule(pattern="*.hackerone.com", rule_type=RuleType.WILDCARD),
                ScopeRule(pattern="hackerone.com", rule_type=RuleType.EXACT),
            ],
        )
        module = SubdomainsActiveModule(
            session, target, scope, wordlist=TINY_WORDLIST, resolvers=RESOLVERS
        )
        result = await module.run()

        assert result.stats["errors"] == {}, f"tool errors: {result.stats['errors']}"
        assert result.stats["in_scope"] > 0, f"no hosts found: {result.stats}"

        rows = (await session.execute(Subdomain.__table__.select())).all()
        assert len(rows) > 0

    await engine.dispose()
